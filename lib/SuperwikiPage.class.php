    ...
    public function wikiFormat()
    {
        ...
        foreach ($pages as $page) {
            if (($page->getId() !== $this->getId()) && $page->isReadable()) {
                $text = preg_replace("/(\s)".preg_quote($page['name'], "/")."/", '$1<a href="'.URLHelper::getLink("plugins.php/superwiki/page/view/".$page->getId(), array('cid' => $page['seminar_id'])).'">'.Assets::img("icons/16/blue/".$page->settings['link_icon'], array('class' => "text-bottom"))." ".htmlReady($page['name']).'</a>', $text);
            }
        }
